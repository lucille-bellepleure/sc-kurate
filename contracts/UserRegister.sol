...
contract UserRegistry {
	...
	event UserAdded(address userAddress, string userName, string userAvatar, string publicKey);

	event VerificationAdded(address _from, address _to, uint256 _numverifications);
	...
	function createUser(
		...
		require(users[msg.sender].numVerifications >= verificationthresh, 'Only verified users can add new users');
	...
	function addVerification(address userAddress) public returns (string memory _feedback) {
		...
		// Maybe here we  can mint and send "erc20 likes"

		emit VerificationAdded(msg.sender, userAddress, user.numVerifications);
	}

	function giveGodVerification(address payable userAddress) public onlyOwner returns (bool) {
		User storage user = users[userAddress];
	...
	function checkVeracity(address payable userAddress) public view returns (uint256 numVerifications) {
