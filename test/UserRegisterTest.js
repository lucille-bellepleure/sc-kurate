...
describe("UserRegistry", function () {
    ...
    it('Should not be able to set a new useraccount from unverified account', async () => {
        ...
        await expect(
            userRegistry.connect(addr1).createUser(
                michelle.account,
                michelle.publicKey,
                michelle.userName,
                michelle.userAvatar
    ...
    it('Should set a new useraccount (Michelle) from Verified user (Owner)', async () => {
        ...
        let user = await userRegistry.connect(owner).createUser(
            michelle.account,
            michelle.publicKey,
            michelle.userName,
            michelle.userAvatar,
    ...
    it('Owner can give user godlike verification to Michelle', async () => {
        const [owner] = await ethers.getSigners();
        let user = await userRegistry.connect(owner).giveGodVerification(
            michelle.account
        );
    ...
    it('Should set a new useraccount (Kiki) from Verified user (Michelle)', async () => {
        const [owner, addr1] = await ethers.getSigners();
        let user = await userRegistry.connect(addr1).createUser(
            kiki.account,
            kiki.publicKey,
            kiki.userName,
            kiki.userAvatar,
    ...
    it('Edina should not be able to verify Kiki', async () => {
        ...
        await expect(userRegistry.connect(addr3).addVerification(kiki.account)).to.be.revertedWith("Only verified users can add new users");
    });
